...
class QueryRoute {
    ...
    router() {
        ...
        router.get('/:id', this.queryID.bind(this));
        return router;
    }
    ...
    async queryID(req, res) {
        ...
        if (!id) ResponseHandler.response(res, 400, 'No Id Found');
        if (!id.match(/^[0-9a-fA-F]{24}$/)) ResponseHandler.response(res, 400, 'Invalid Id');
        while (retries < 3) {
            try {
