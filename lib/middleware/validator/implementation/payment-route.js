...
class Validator {
    ...
    static checkPayloadA(payload) {
        ...
        if (!Validator.checkCCVLength(_.get(payload, 'credit_card.ccv'))) {
            _.set(result, 'validity', false);
            _.set(result, 'message', 'Invalid CCV format');
        }
        return result;
    }
    ...
    static checkCCVLength(ccv) {
        // clean the credit card number first
        const cleaned_ccv = ccv.replace(/\D/g, '');
        return cleaned_ccv.length === 3;
    }
}
